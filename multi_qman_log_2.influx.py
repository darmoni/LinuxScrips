...
class log_collector:
    ...
    def run(self, execute_cmd='~/bin/staging_qman_2_influxdb.sh'):
        (remote_log_srv_cmd,log_srv_cmd,parser,influx_writer) = self._params
        self.setup()
        try:
            if(-1 == self._p): return
...
try:
    ...
    conf_log_collector.run('~/bin/staging_qman_2_influxdb.sh')
    #check_output('~/bin/staging_conf_only.sh')
    ...
    mserver_log_collector._p.kill()
    db_log_collector._p.kill()
