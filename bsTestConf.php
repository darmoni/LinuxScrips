...
function call_bridge($cmdline, $count, $conference, $bridge) {
    echo "joining conference $conference,$bridge#\n";
    if($count<= 0) return;
    for($i= $count;$i >0; $i--) {
        fwrite($cmdline, "dsip:$conference\n");
        sleep (9);
        fwrite($cmdline, "$bridge\n");
        sleep (2);
        fwrite($cmdline, "#\n");
        sleep (2);
        fwrite($cmdline, "#\n");
        sleep(5);
        if(1 < $count) fwrite($cmdline, "T");  // switch agent
    }
}
...
//    $process = proc_open('/usr/local/bin/baresip', $descriptorspec, $pipes/*, $cwd, $env*/);
    ...
    $cmdline = $pipes[0];
            ...
            call_bridge($cmdline, 1, "888", "11111");/*
            for ($i=0 ; $i < $count; $i++) {
